...
while(True):                 # we'll limit ourselves to 5 seconds.
    ...
    with open('/tmp/tcp-lte.dat', 'w') as f:
        p = subprocess.Popen(['/usr/bin/tcpstat', '-r', 'iperf-mptcp-0-0.pcap', '-o',  '%R %b\n', '1'], 
                             stdout=f)
        p.wait ()
    subplot(311)
    ...
    with open('/tmp/tcp-wifi.dat', 'w') as f:
        p = subprocess.Popen(['/usr/bin/tcpstat', '-r', 'iperf-mptcp-0-1.pcap', '-o',  '%R %b\n', '1'], 
                             stdout=f)
        p.wait ()
    subplot(312)
    ...
    with open('/tmp/tcp-aggr.dat', 'w') as f:
        p = subprocess.Popen(['/usr/bin/tcpstat', '-r', 'iperf-mptcp-1-0.pcap', '-o',  '%R %b\n', '1'], 
                             stdout=f)
        p.wait ()
    subplot(313)
